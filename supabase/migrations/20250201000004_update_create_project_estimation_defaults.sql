-- ============================================================================
-- UPDATE create_project_estimation RPC FUNCTION
-- ============================================================================
-- Updates the function to set default values for new projects:
--   - estimate_status = 'APPROVED' (instead of 'PENDING')
--   - is_project_requested = true (instead of false)
--   - is_invoice_requested = false (instead of false)
-- ============================================================================
...
BEGIN
  ...
  INSERT INTO public.project_estimation_table (
    ...
    drafted_json,
    estimate_status,
    is_project_requested,
    is_invoice_requested,
    created_at,
    updated_at
  ) VALUES (
    ...
    '[]'::jsonb,
    NULL,
    CASE WHEN v_existing_project_uuid IS NULL THEN 'PRE-PROD' ELSE (SELECT project_status FROM public.project_estimation_table WHERE project_estimate_uuid = v_existing_project_uuid) END,
    COALESCE(p_is_drafted, true),
    COALESCE(p_drafted_json, '{}'::jsonb),
    -- Set default values for new projects only (not updates)
    CASE WHEN v_existing_project_uuid IS NULL THEN 'APPROVED' ELSE (SELECT estimate_status FROM public.project_estimation_table WHERE project_estimate_uuid = v_existing_project_uuid) END,
    CASE WHEN v_existing_project_uuid IS NULL THEN true ELSE (SELECT is_project_requested FROM public.project_estimation_table WHERE project_estimate_uuid = v_existing_project_uuid) END,
    CASE WHEN v_existing_project_uuid IS NULL THEN false ELSE (SELECT is_invoice_requested FROM public.project_estimation_table WHERE project_estimate_uuid = v_existing_project_uuid) END,
  ...
  DO UPDATE SET
    ...
    updated_at = now();
    -- Note: estimate_status, is_project_requested, and is_invoice_requested are NOT updated on conflict
    -- They maintain their original values when updating an existing project
...
For new projects, sets estimate_status=''APPROVED'', is_project_requested=true, is_invoice_requested=false.';
