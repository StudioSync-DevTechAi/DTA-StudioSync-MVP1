 ...
 */
const GridPattern = ({ offsetX, offsetY, size, patternId = "grid-pattern", isDark = true }: { offsetX: any; offsetY: any; size: number; patternId?: string; isDark?: boolean }) => {
  return (
      ...
      <defs>
        ...
        >
          <path
            ...
            stroke="currentColor"
            strokeWidth="1"
            className={isDark ? "text-white/40" : "text-black/20"} 
          />
        </motion.pattern>
      </defs>
...
/**
 * The Infinite Grid Component - Background only version
 * Displays a scrolling background grid with a flashlight effect on mouse hover.
 * Use this for background layers.
 */
export const InfiniteGrid = ({ theme = "dark" }: { theme?: "dark" | "light" }) => {
  ...
  // Use useEffect to track mouse movement on the entire document
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 opacity-[0.05]">
        <GridPattern offsetX={gridOffsetX} offsetY={gridOffsetY} size={gridSize} patternId="grid-pattern-bg" isDark={isDark} />
      </div>
      ...
      >
        <GridPattern offsetX={gridOffsetX} offsetY={gridOffsetY} size={gridSize} patternId="grid-pattern-highlight" isDark={isDark} />
      </motion.div>

      {/* Decorative Blur Spheres */}
      <div className="absolute inset-0 pointer-events-none z-0">
        {/* Top right - Orange sphere */}
        <div className={`absolute right-[-20%] top-[-20%] w-[40%] h-[40%] rounded-full ${isDark ? "bg-orange-500/40" : "bg-orange-400/30"} blur-[120px]`} />
        {/* Top right - Smaller primary sphere */}
        <div className={`absolute right-[10%] top-[-10%] w-[20%] h-[20%] rounded-full ${isDark ? "bg-purple-500/30" : "bg-purple-400/20"} blur-[100px]`} />
        {/* Bottom left - Blue sphere */}
        <div className={`absolute left-[-10%] bottom-[-20%] w-[40%] h-[40%] rounded-full ${isDark ? "bg-blue-500/40" : "bg-blue-400/30"} blur-[120px]`} />
      </div>
    </div>
  );
};
...
/**
 * The Infinite Grid Component - Full version with controls and content
 * Displays a scrolling background grid that reveals an active layer on mouse hover.
 * Includes demo content and controls.
 */
export const InfiniteGridFull = () => {
  ...
  return (
    ...
    >
      {/* Layer 1: Subtle background grid (always visible) */}
      <div className="absolute inset-0 z-0 opacity-[0.05]">
        <GridPattern offsetX={gridOffsetX} offsetY={gridOffsetY} size={gridSize} patternId="grid-pattern-full-bg" />
      </div>
      ...
      <motion.div 
        ...
        style={{ maskImage, WebkitMaskImage: maskImage }}
      >
        <GridPattern offsetX={gridOffsetX} offsetY={gridOffsetY} size={gridSize} patternId="grid-pattern-full-highlight" />
      </motion.div>
...
// Default export for backward compatibility - exports the background-only version
const InfiniteGridComponent: React.FC = () => {
